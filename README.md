...
Local Review Task resolution and Current Priority overrides use the stable `demo-operator` identity. Enter the local Operator access key `local-demo-key` in the review form, or set `OPERATOR_KEY` before startup to use a different key. The key is sent only with the command and is not persisted by the app. For a public deployment, set `PUBLIC_DEMO_READ_ONLY=true` on both API and web services; this disables mutation routes and hides operator controls.
